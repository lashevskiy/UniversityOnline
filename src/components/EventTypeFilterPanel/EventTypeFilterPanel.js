...
class EventTypeFilterPanel extends React.Component {
    ...
    renderData() {
        ...
        let eventTypeArray = this.cityList;

        return (
            <React.Fragment>
                <Search value={this.state.search} onChange={this.onChange}/>
                {eventTypeArray.length > 0 && !this.props.isLoading && (
                    <List>
                        {eventTypeArray.map(city => {
                            return (
                                <Cell key={city.slug}
                                      multiline
                                      asideContent={this.props.eventTypeId.slug === city.slug ? <Icon24Done fill="#528bcc" /> : null}
                                      onClick={(e) => {
                                          this.props.actionsFilterPanel.setEventTypeId(city);
                                          this.props.go(e);
                                      }}
                                      data-to={'FilterPanel'}>
                                    {city.name}
    ...
    render() {
        ...
        //console.log('EventTypeFilterPanel', this.props)
...
const mapStateToProps = store => {
    return {
        ...
        isLoading: store.eventTypeFilterPanel.isLoading,
        isHasError: store.eventTypeFilterPanel.isHasError,
    }
};
